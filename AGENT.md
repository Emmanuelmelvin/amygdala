...
The memory gateway checks the VC object on every query. Revocation is immediate — flip the flag on-chain, and the next query fails.
...
6. Sign access-log transaction
   ├── Construct AccessLog event payload
   ├── Sign with session key
   └── Submit to SUI (fire-and-forget, async — do not block retrieval on tx confirm)
   │
...
The indexer is **re-derivable from chain history**. If the vector index is corrupted or lost, all `MemoryChunkRef` objects on-chain can be replayed to re-fetch and re-embed the content from the DA layer and rebuild the index. The chain is the canonical source of which chunks exist; the index is a derived, queryable view.
